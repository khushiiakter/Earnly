          ...
          <div className="max-w-6xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 py-10 gap-6">
              {roles.map((role) => (
                <div
                  key={role.name}
                  className=" p-6 flex flex-col justify-between rounded-lg shadow-2xl hover:shadow-2xl border- border"
                  aria-label={`${role.name} role`}
                >
                  <h3 className="text-3xl text-center font-bold mb-4 text-[#8e60ac]">
                    {role.name}
                  </h3>
                  <p className="text-sm  mb-6">{role.description}</p>
                  <ul className="text-sm  mb-4">
                    {role.features.map((feature, index) => (
                      <li key={index} className="mb-2 flex items-center">
                        <span className="text-[#8e60ac] mr-2" aria-hidden="true">✔</span>
                        <span>{feature}</span>
                      </li>
                    ))}
                  </ul>
                  <button className="btn btn-primary text-white border-none bg-[#5f1a89] hover:bg-[#0F1035] w-full">
                    Get Started
                  </button>
                </div>
              ))}
